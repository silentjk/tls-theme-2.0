...
<div id="university">
	...
	<div class="row">
		<?php
			$args = array( 'posts_per_page' => 1, 'offset'=> 0, 'category' => 4, 'tag_id' => 452 );
		...
		?>
		<?php
			$args = array( 'posts_per_page' => 1, 'offset'=> 1, 'category' => 4, 'tag_id' => 452 );
			$myposts = get_posts( $args );
			foreach ( $myposts as $post ) : setup_postdata( $post ); 
		?>
		...
		<div id="univ-stories" class="col-xs-3">
			<?php
				$args = array( 'posts_per_page' => 5, 'offset'=> 2, 'category' => 4, 'tag_id' => 452 );
				$myposts = get_posts( $args );
				foreach ( $myposts as $post ) : setup_postdata( $post ); 
			?>
			<div class="row">
